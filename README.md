...
For signings, the owner privately selects a player from the correct club role or chooses **Player not listed**, supplies verified facts, and then the correct reporter requests a genuine quote and an optional player photo by DM. If no photo is supplied, the bot generates club-themed signing artwork without inventing a player likeness. The player can submit a quote or decline. A 12-hour timeout sends the owner options to continue without a quote, enter one manually, or cancel. Tru’s configured user ID uses his saved quote and publishes without waiting.
