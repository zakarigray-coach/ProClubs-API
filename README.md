...
The bot publishes match reports, player signings, weekly recaps, Player Spotlights, awards, and departures as dated RT Football Media front pages. Every edition uses one locked 1024×1536 printed-tabloid grid: distressed paper, floodlit RT MEDIA masthead and crown, weekday/date bar, club-and-league strip, oversized complete headline, narrow news brief, large hero photograph, distinct `INSIDE TODAY` teasers, lower feature panel, storylines rail, and footer. Birmingham City uses the approved upper-page treatment throughout—aged white paper, black copy, and Birmingham blue accents. CrownFC uses the approved lower-page treatment throughout—black/deep-navy paper, white copy, and Carolina-blue accents. Content changes; the grid and club themes do not. Every visible entry is a complete sentence; the renderer never adds ellipses, never cuts a paragraph mid-sentence, and does not repeat the lead summary in the sidebar. Each story receives fresh AI-generated hero artwork, while exact typography and the Eastern-Time publication date are rendered separately. Public posts contain the finished front page without repeating the full article as a long Discord embed.

The permanent design and validation contract is stored in [`RT_MEDIA_MASTER_DESIGN_RULES.md`](RT_MEDIA_MASTER_DESIGN_RULES.md). Player Spotlight is a separate premium magazine/editorial renderer and never uses the newspaper layout.

## Commands

- `/match` — required match graphic plus optional extra context
- `/sign` — one-player reporter-led signing workflow
- `/sign-batch` — one unified signing-class announcement for two to five signings, with an optional explicitly selected marquee player
...
`/sign-batch` selects two to five unsigned players from one club. Management may explicitly identify one of those selected players as the marquee signing; the bot never chooses one automatically. Management enters each player as `position | previous club`; each non-Tru player privately selects their own protected number and supplies their own genuine quote and optional photo. After every player is ready, the bot generates one unified signing-class image—not separate mini-posters—plus one RT Football Media newspaper. The owner preview specifically requires checking every face and left-to-right name/position match. One approval creates one transactions post containing the unified graphic, one newspaper post and one club-role mention. Cancelling publishes nothing and assigns no numbers.
...
The audit explicitly identifies redundant categories named `Start Here`, `Club`, `Club Info`, `Club Information`, or `Club Info & Community`. After owner approval, essential welcome channels move to Welcome, club-specific channels move into the current club structure, empty redundant channels are deleted, history-bearing leftovers move privately to Club Archive, and resulting empty category shells are removed. The existing `Community` category is renamed `C&C COLLECTIVE`; a shared `locker-room-chat` is retained there as `collective-clubhouse` so players from both clubs can talk and organize casual play. If an earlier `C&C COMMUNITY` duplicate exists, its channels are consolidated into `C&C COLLECTIVE` and the empty duplicate is removed. Generic `roster-polls` is retired because registration and management approval replace it; existing visible history is archived rather than destroyed.
